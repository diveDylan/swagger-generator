    ...
    function transDefinintion(properties) {
        return Object
            ...
            if (langMap[property.type]) {
                return const_1.makeInter(property.description, item, langMap[property.type], requiredList.includes(item));
            }
            if (property.$ref || property.type == 'array') {
                // deal ref link
                if (property.$ref) {
                    const interfaceName = getInterfaceByRef(property.$ref, item, property, modelsList);
                    return const_1.makeInterTypes(property.description, item, interfaceName, requiredList.includes(item));
                }
                else { // array
                    if (property.items.$ref) { // array with ref link
                        ...
                        return const_1.makeInterArr(property.description, item, interfaceName, requiredList.includes(item));
                    }
                    else {
                        // transDefinition property.item
                        return const_1.makeDepInter(property.description, item, getArrayInterface(property.items, langMap), requiredList.includes(item));
                    }
                }
            }
            return const_1.makeInter(property.description, item, property.type, requiredList.includes(item));
        }).join('');
    }
    return {
...
function getArrayInterface(arrayItem, langMap) {
    return Object.keys(arrayItem).map(item => {
        ...
        if (langMap[property.type]) {
            return const_1.makeInter(property.description, item, langMap[property.type], true);
        }
        return const_1.makeInter(property.description, item, property.type, true);
    }).join('');
}
